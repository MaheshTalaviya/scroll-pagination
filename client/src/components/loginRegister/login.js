...
const Login = () => {
    ...
    useEffect(()=>{
        ...
        if(user_type!==null && isAuthenticated){
            ...
            if(user_type==='LEAD'){
                 history.push("/v1/agent/leadAgent");
            }else if(user_type==='DUP'){
                 history.push("/v1/agent/dupCheckAgent");
            }else if(user_type==='OUTREACH'){
                history.push("/v1/agent/outReachAgent");
            }else if(user_type==='ADMIN1'){
                history.push("/admin1/uploadUsers");
            }else{
                history.push("/v1/admin2/leadGen");
            }
        }
    },[]);
    ...
    try {
        res = await axios.post(SERVER_URL + "/api/userAgentsusers/login", {
            email: email,
            password: password
        });
        console.log("res", res.data);
        
        if (res.data.success === true && res.data.user.user_type === "LEAD") {
            const token = res.data.token;
            const user = res.data.user;
            const user_type = res.data.user.user_type;
            window.localStorage.setItem("lead_token", token);
        ...
        else if(res.data.success === true && res.data.user.user_type === "DUP"){
             const token = res.data.token;
            const user = res.data.user;
            const user_type = res.data.user.user_type;
            window.localStorage.setItem("lead_token", token);
        ...
        else if(res.data.success === true && res.data.user.user_type === "OUTREACH"){
             const token = res.data.token;
            const user = res.data.user;
            const user_type = res.data.user.user_type;
            window.localStorage.setItem("lead_token", token);
            ...
            window.localStorage.setItem("user", JSON.stringify(user));
            window.localStorage.setItem("user_type", user_type);
             window.localStorage.setItem("route", 'admin1');
            window.localStorage.setItem("isAuthenticated", true);
            setAuthToken(); //  Important set token  from res.data to get token
            dispatch({type: "IS_AUTHENTICATED"});
            dispatch({type: "USER_DATA",payload:user});
           ...
           window.localStorage.setItem("user", JSON.stringify(user));
           window.localStorage.setItem("user_type", user_type);
             window.localStorage.setItem("route", 'admin2');
           window.localStorage.setItem("isAuthenticated", true);
          setAuthToken(); //  Important set token  from res.data to get token
          dispatch({type: "IS_AUTHENTICATED"});
          dispatch({type: "USER_DATA",payload:user});
